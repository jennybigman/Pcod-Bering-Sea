	...
	year_bins_hist <- list('dec_1985_1989' = c(1985:1989),
                    		 'dec_1990_1994' = c(1990:1994),
                    		 'dec_1995_1999' = c(1995:1999),
                    		 'dec_2000_2004' = c(2000:2004),
  											 'dec_2005_2009' = c(2005:2009),
  											 'dec_2010_2014' = c(2010:2014))
  ...
  df_proj_hist_long <- df_proj_hist %>%
  	gather(pct_change, value, -c("latitude", "longitude", "simulation", "projection")) 
  
  df_proj_hist_long_sf <- df_proj_hist_long %>%
  	mutate(long_not_360 = longitude - 360) %>%
  	st_as_sf(coords = c("long_not_360", "latitude"), crs = 4326, remove = FALSE)
  
  df_proj_hist_long_sf <- df_proj_hist_long_sf %>%
  	rename(time_period = pct_change,
  				 pct_change = value)
  
  df_proj_hist_long_sf$time_period <- str_replace(df_proj_hist_long_sf$time_period,
  																						 "pct_change_", "")
  
   	
  	plot <- 
  		ggplot() +
					geom_sf(data = df_proj_hist_long_sf, aes(color = pct_change))  +
  	...
  	ggsave("./output/plots/percent change_projhist.png",
		plot,
