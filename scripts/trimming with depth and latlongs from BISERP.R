	...
	ROMS_dat_hind_trim <- temp_hind_dat %>%
...
#		width = 10, height = 10, units = "in")
	...
	## check
 		...
 		ggplot() +
		geom_sf(data = ROMS_hindcast_temp_dat_sf, aes(color = temp))  +
		geom_sf(data = world_map_data, fill = "darkgrey", lwd = 0) +
		coord_sf(crs = 3338) +
		scale_color_viridis_c() +
 		scale_x_continuous(
	...
	# now filter by depth
		...
		ROMS_hindcast_temp_dat2 <- ROMS_hindcast_temp_dat %>%
			mutate_at(vars, as.character)
		
		ROMS_dat_hind_depth <- full_join(ROMS_hindcast_temp_dat2, depth_df)

		ROMS_dat_hind_depth <- ROMS_dat_hind_depth %>%
			filter(., between(depth, 0, 250))
	...
	ROMS_dat_hind_depth_sum <-  ROMS_dat_hind_depth %>%
		group_by(latitude, longitude) %>%
		summarise(mean_temp = mean(temp))
	
	ROMS_dat_hind_depth_sum_sf <- ROMS_dat_hind_depth_sum	%>% 
		mutate(long_not_360 = case_when(
					 longitude >= 180 ~ longitude - 360,
					 longitude < 180 ~ longitude))  %>%
  	st_as_sf(coords = c("long_not_360", "latitude"), crs = 4326, remove = FALSE)
 	...
 	ggplot() +
		geom_sf(data = ROMS_dat_hind_depth_sum_sf, aes(color = mean_temp))  +
		geom_sf(data = world_map_data, fill = "darkgrey", lwd = 0) +
		coord_sf(crs = 3338) +
		scale_color_viridis_c() +
 		scale_x_continuous(
