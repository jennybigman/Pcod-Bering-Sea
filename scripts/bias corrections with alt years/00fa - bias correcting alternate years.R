...
# bias correction
	...
	proj_temp_dat_all_difrefper <- bind_rows(cesm_bc_temps, gfdl_bc_temps, miroc_bc_temps) ## this is not trimmed
	
	fwrite(proj_temp_dat_all_difrefper, file = here("./data/ROMS_proj_temp_dat_all_difrefper.csv"))
	

	
	## remove grid cells based on depth and location (keep those < 250 m and within Ortiz regions)
	
	proj_temp_dat_all_difrefper_sum <- proj_temp_dat_all_difrefper %>%
		group_by(Lat, Lon) %>%
  ...
  ROMS_hindcast_dat_sum <- ROMS_hindcast_dat %>%
	...
	dat_ints <- st_intersection(ROMS_hindcast_dat_sum, proj_temp_dat_all_difrefper_sum)
	...
	proj_temp_dat_difrefper <- merge(proj_temp_dat_difrefper, area_depth_df, by = c("latitude", "longitude"))

	fwrite(proj_temp_dat_difrefper, "./data/proj_temp_dat_difrefper.csv")

	#### time series plot ####
	proj_temp_dat_difrefper <- fread("./data/proj_temp_dat_difrefper.csv")
	...
	sim_proj <- unique(yearly_temp_dat_proj_drp$sim_proj)
	
	names(colors) <- unique(yearly_temp_dat_proj_drp$sim_proj)
	
	proj_temp_plots_sd_drp <-    
   	ggplot() +
  	...
  	theme(
		...
		ggsave("./output/plots/proj_temp_plots_sd_drp.png",
			 proj_temp_plots_sd_drp,
