...
proj_temp_plots_sd_compare <-    
	 ...
	 proj_temp_plots_sd_compare <- proj_temp_plots_sd_compare +
	  	geom_text(data = data.frame(x = 2011.7, y = -1.3, 
	  															simulation = "cesm", projection = "historical",
	  															label = "ref period:"),
	  					  aes(x = x, y = y, label = label), color = "black", size = 4) +
	  	geom_text(data = data.frame(x = 2012, y = -1.70, 
	  															simulation = "cesm", projection = "historical",
	  															label = "1980 - 2014"),
	  					  aes(x = x, y = y, label = label), color = "#1E90FF", size = 4) +
	  	geom_text(data = data.frame(x = 2012, y = -2.05, 
	  															simulation = "cesm", projection = "historical",
	  															label = "1990 - 2014"),
	  					  aes(x = x, y = y, label = label), color = "#ff8d1e", size = 4)
	  
	   ggsave("./output/plots/proj_temp_plots_sd_compare.png",
			 proj_temp_plots_sd_compare,
