...
#### panel a: map of historical averaged temps in march ####
	...
	march_avg_temps_hind <-
		...
		coord_sf(crs = 3338) +
		scale_color_viridis_c(limits = c(-2.2, 12.5)) +
 		scale_x_continuous(
 		 breaks = c(-170, -160),
		 labels = c("-170˚", "-160˚"),
	...
	#### panel b: map of projected averaged temps in march avg across models ####
	...
	### panel d: hatch success & temperature relationship ####
 ...
 plot2 <- march_avg_temps_proj_ssp126 + theme(plot.margin = unit(c(0.2, 0, 0.2, -0.05), "in"))
