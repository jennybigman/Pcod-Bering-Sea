# area metric 
	...
	area_plot_yr <-    
	  ...
	  #name = expression(paste("Total area x",' '(10^{5}),' '(km^{2}))),
	  	  #name = expression(paste("Total area x",' '(km^{2}))),
    ...
    theme_bw() +
  	...
  	theme(
		...
		## for black background plot
		
			area_plot_yr_white <-    
   	ggplot(data = hind_area_yr) +
	 	geom_line(aes(year, total_area), alpha = 0.5,
            data = . %>% filter(sp_hab_threshold == "potential"), color = "grey") +
		geom_vline(aes(xintercept = 2020), color = "grey", size = 0.15) +
		geom_line(data = proj_area_yr, 
							aes(year, total_area, color = sim_proj, group = sim_proj, alpha = 0.5)) +
		facet_grid(sp_hab_threshold ~ projection) +
	  geom_line(aes(year, total_area, colour = sp_hab_threshold, alpha = 0.5),
            data = . %>% filter(sp_hab_threshold == "core"), color = "grey") +
		xlab("Year") +
		scale_color_manual(name = "sim_proj", values = colors) +
	  scale_y_continuous(
	  	breaks = c(0, 200000, 400000),
    ...
    theme_bw() +
  	...
  	theme(
		...
		ggsave("./output/plots/area_plot_yr_white.png",
			 area_plot_yr_white,
			 width = 10, height = 5, units = "in")
