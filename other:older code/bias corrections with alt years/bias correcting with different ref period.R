# boas-correction at domain/strata level, not grid cell level for a different time period
...
# bias correction
	...
	baseline_years <- 1990:2014 # define baseline/ref years (here based on Cheng et al 2021) 
...
#
	...
	cesm_bc_temps_difrefper <- cesm_bc_temps %>%
	...
	fwrite(cesm_bc_temps_difrefper, file = "./data/cesm_bc_temps_difrefper.csv")
...
#		summarise(mo_avg_proj_temp = mean(temp))
	...
	gfdl_bc_temps_difrefper <- gfdl_bc_temps %>%
		mutate(sd_ratio = sd_mo_baseline_temp/sd_proj_baseline,
					 bc_temp = delta + mean_mo_baseline_temp,
					 bc_temp_sd = (sd_ratio * delta) + mean_mo_baseline_temp)
	
	fwrite(gfdl_bc_temps_difrefper, file = "./data/gfdl_bc_temps_difrefper.csv")
	...
	miroc_bc_temps_difrefper <- miroc_bc_temps %>%
		mutate(sd_ratio = sd_mo_baseline_temp/sd_proj_baseline,
					 bc_temp = delta + mean_mo_baseline_temp,
					 bc_temp_sd = (sd_ratio * delta) + mean_mo_baseline_temp)

	
	fwrite(miroc_bc_temps_difrefper, file = "./data/miroc_bc_temps_difrefper.csv")

	#### add together ####
	cesm_bc_temps_difrefper <- fread(file = "./data/cesm_bc_temps_difrefper.csv")
	gfdl_bc_temps_difrefper <- fread(file = "./data/gfdl_bc_temps_difrefper.csv")
	miroc_bc_temps_difrefper <- fread(file = "./data/miroc_bc_temps_difrefper.csv")

	cesm_bc_temps_difrefper$simulation <- "cesm"
	gfdl_bc_temps_difrefper$simulation <- "gfdl"
	miroc_bc_temps_difrefper$simulation <- "miroc"
	
	cesm_bc_temps_difrefper$projection <- cesm_bc_temps_difrefper$scenario
	gfdl_bc_temps_difrefper$projection <- gfdl_bc_temps_difrefper$scenario
	miroc_bc_temps_difrefper$projection <- miroc_bc_temps_difrefper$scenario

	proj_temp_dat_all_difrefper <- bind_rows(cesm_bc_temps_difrefper, gfdl_bc_temps_difrefper, miroc_bc_temps_difrefper) ## this is not trimmed
	
	fwrite(proj_temp_dat_all_difrefper, file = here("./data/proj_temp_dat_all_difrefper.csv"))
	
	#proj_temp_dat_all <- fread(file = here("./data/ROMS_proj_temp_dat_all.csv"))
	
	# trim by depth -- keep < 250 m
	proj_temp_dat_all_difrefper <- proj_temp_dat_all_difrefper %>%
		mutate(latitude = Lat,
					 longitude = Lon)
	
	proj_temp_dat_all_difrefper <- merge(area_depth_df, proj_temp_dat_all_difrefper,
														 by = c("latitude", "longitude"))
	
	proj_temp_dat_difrefper <- proj_temp_dat_all_difrefper %>%
		filter(., between(depth, 0, 250))


	fwrite(proj_temp_dat_difrefper, "./data/proj_temp_dat_difrefper.csv")
