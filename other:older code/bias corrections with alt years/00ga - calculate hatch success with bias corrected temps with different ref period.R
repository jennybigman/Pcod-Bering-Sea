# calculating hatch success	
	...
	# projected data
	ROMS_proj_temp_dat_drp <- fread(file = here("./data/proj_temp_dat_difrefper.csv"))
	...
	ROMS_proj_temp_dat_drp <- ROMS_proj_temp_dat_drp %>%
  ...
  ROMS_hindcast_temp_dat <- ROMS_hindcast_temp_dat %>%
	...
	ROMS_projected_dat_drp <- ROMS_proj_temp_dat_drp %>%
  	mutate(sp_hab_suit = hatch_success/max(hatch_success))
	 
	 ROMS_projected_dat_drp_sum <- ROMS_projected_dat_drp %>%
	 	group_by(simulation, projection, year) %>%
	 	summarise(n())
  
	fwrite(ROMS_projected_dat_drp, file = "./data/ROMS_projected_dat_drp.csv")
