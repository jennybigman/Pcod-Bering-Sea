# ROMS proj temps domain-level BC not trimmed to months
...
# read in netcdf files of projected temp from Bering 10k ROMS
 ...
 # ssp585 scenario ####
  ...
  gfdl_ssp585_dfs <- bind_rows(gfdl_ssp585_df_list)
	...
	fwrite(gfdl_dfs, "./data/gfdl_dfs.csv")
 ...
 # ssp585 scenario ####
  ...
  miroc_ssp585_dfs <- bind_rows(miroc_ssp585_df_list)
	...
	fwrite(miroc_dfs, "./data/miroc_dfs.csv")
  ...
  #### bias correction ####
	...
	gfdl_dfs <- fread("./data/gfdl_dfs.csv")
	miroc_dfs <- fread("./data/miroc_dfs.csv")
	...
	cesm_dfs <- merge(cesm_dfs, domain_df,
										by = c("Xi", "Eta")) 
