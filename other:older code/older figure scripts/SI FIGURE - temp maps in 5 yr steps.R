# SI figure of temps every 5 years
	...
	#### temps bias corrected at monthly, regional level ####
  ...
  names_file <- sapply(names_plots, name_func_file)
   ...
   ggsave_func <- function(x,y){
	...
	#### temps bias corrected at weekly, grid cell level ####
	...
	# annual average
	ROMS_projected_yr_sum <- proj_mo_dfs %>%
		filter(year %in% years_proj) %>%
		group_by(simulation, scenario, latitude, longitude, year) %>%
		summarise(mean_temp = mean(bc_temp_mo))
	
	ROMS_projected_5yr_sum <- ROMS_projected_yr_sum %>%
		mutate(ints = cut(year, breaks = 15)) %>%
		group_by(simulation, scenario, ints, latitude, longitude) %>%
		summarize(mean_temp = mean(mean_temp)) %>%
		mutate(long_not_360 = case_when(
					 longitude >= 180 ~ longitude - 360,
					 longitude < 180 ~ longitude))  %>%
	...
	ROMS_projected_5yr_sum_test <- ROMS_projected_5yr_sum %>%
			group_by(simulation, scenario, ints, latitude, longitude) %>%
			summarize(mean_temp = mean(mean_temp))
	...
	proj_temp_plot_func <- function(x){
		...
		plot <- 
   	  	ggplot() +
				...
				coord_sf(crs = 3338) +
				scale_color_viridis_c(limits = c(-2.14, 10.53)) +
   	  	facet_grid(scenario ~ simulation + ints) +
 				scale_x_continuous(
 					breaks = breaks_x,
 					labels = labels_x,
 					name = "Longitude",
  ...
  name_func <- function(x){
  	year_month <- paste0(x, "_proj_temp_5yr_sum_plot_wkgc")
  }
   
  names_plots <- sapply(sims, name_func)
  
	name_func_file <- function(x){
  	year_month <- paste0("/Users/jenniferbigman/Dropbox/NOAA AFSC Postdoc/Pcod Project/Pcod Bering Sea Habitat Suitability/Pcod-Bering-Sea/scripts/with weekly grid cell level bias correct temps/", x)
  }
  ...
  names_file <- sapply(names_plots, name_func_file)
   ...
   ggsave_func <- function(x,y){
	...
	ggsave("./scripts/with weekly grid cell level bias correct temps/proj_5yr_temp_plot_combined.png",
			 proj_5yr_temp_plot_combined,
			 width = 15, height = 10, units = "in")

		
	# for some reason above isnt working so plot separately
	
		cesm_dat <- ROMS_projected_5yr_sum %>% filter(simulation == "cesm")
		
		cesm_plot <- 
   	  	ggplot() +
				geom_sf(data = cesm_dat, aes(color = mean_temp))  +
				geom_sf(data = world_map_data, fill = "grey", lwd = 0) +
				coord_sf(crs = 3338) +
				scale_color_viridis_c(limits = c(-2.14, 10.53)) +
   	  	facet_grid(scenario ~ simulation + ints) +
  ...
  ggsave(here("./scripts/with weekly grid cell level bias correct temps/cesm_proj_temps_5yr.png"),
			 cesm_plot,
			 height = 5,
			 width = 10)
  ...
  gfdl_dat <- ROMS_projected_5yr_sum %>% filter(simulation == "gfdl")
		
	gfdl_plot <- 
   	  	ggplot() +
				geom_sf(data = gfdl_dat, aes(color = mean_temp))  +
				geom_sf(data = world_map_data, fill = "grey", lwd = 0) +
				coord_sf(crs = 3338) +
				scale_color_viridis_c(limits = c(-2.14, 10.53)) +
   	  	facet_grid(scenario ~ simulation + ints) +
  ...
  ggsave(here("./scripts/with weekly grid cell level bias correct temps/gfdl_proj_temps_5yr.png"),
			 gfdl_plot,
			 height = 5,
			 width = 10)
  ...
  ## miroc
  	...
  	miroc_dat <- ROMS_projected_5yr_sum %>% filter(simulation == "miroc")
		
		miroc_plot <- 
   	  	ggplot() +
				geom_sf(data = miroc_dat, aes(color = mean_temp))  +
				geom_sf(data = world_map_data, fill = "grey", lwd = 0) +
				coord_sf(crs = 3338) +
				scale_color_viridis_c(limits = c(-2.14, 10.53)) +
   	  	facet_grid(scenario ~ simulation + ints) +
  ...
  ggsave(here("./scripts/with weekly grid cell level bias correct temps/miroc_proj_temps_5yr.png"),
			 miroc_plot,
			 height = 5,
			 width = 10)
