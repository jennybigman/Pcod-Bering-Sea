# calculating the mean latitude of spawning habitat suitability
	...
	mean_lat_func <- function(dat){
    
				new_dat05 <- dat %>%
					filter(., sp_hab_suit >= 0.5)
		
				new_dat05_sum <- new_dat05 %>%
					group_by(year) %>%
				...
				new_dat09_sum <- new_dat05 %>%
					group_by(year) %>%
					summarise(mean_lat = mean(latitude)) 	
				
				dat_list <- list(new_dat05_sum, new_dat05_sum)
				dat_list
				
     }
		

	dat_list <- list(ROMS_hindcast_dat, ROMS_projected_dat)

	mean_lats_yr <- mapply(FUN = mean_lat_func,
												 dat = dat_list)

	mean_lats_yr_0.5 <- bind_rows(mean_lats_yr[[1]], mean_lats_yr[[3]]) %>%
		rename(mean_lat_0.5 = mean_lat)
	
	mean_lats_yr_0.9 <- bind_rows(mean_lats_yr[[2]], mean_lats_yr[[4]])	%>%
